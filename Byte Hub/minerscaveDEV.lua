...
if not getgenv().bytehubLoaded then
  ...
  local sptog = Tabs.lp:AddToggle("Jesus",
  {
    Title = "Sprint", 
  ...
  local spspeed = Tabs.st:AddInput("Input", {
    Title = "Sprint Speed",
    Description = "Speed of sprint",
    Default = "0",
    Placeholder = "Enter a number",
    Numeric = false, -- Ensure input is numeric
    Finished = false,
    Callback = function(si)
      local newSpeed = tonumber(si)
      if newSpeed then
        speed = newSpeed -- Update the delay value dynamically
      else
        Fluent:Notify({
          Title = "Error",
          Content = "Invalid Speed",
          SubContent = "Please enter a number",
          Duration = 3
        })
      end
